...
class NewIssueForm extends TPage
{
    ...
    function __construct()
    {
        ...
        $register_date->setSize('calc(100% - 50px)');
        $file->setSize('100%');
        $time->setSize(50);
        $time->setMask('99:99');
        $title->setSize('100%');
        ...
        $this->form->addFields( [new TLabel(_t('Priority'))], [$id_priority], [new TLabel(_t('Title'))], [$title]);
        $this->form->addFields( [new TLabel(_t('Category'))], [$id_category], [new TLabel(_t('File'))], [$file]);
