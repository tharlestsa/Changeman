...
class UpdateIssueForm extends TPage
{
    ...
    function __construct()
    {
        ...
        $register_date->setSize(120);
        $close_date->setSize(120);
        $time->setSize('50');
        $title->setSize('100%');
        $description->setSize('100%', 200);
        $solution->setSize('100%', 200);
        ...
        $this->form->addFields( [new TLabel(_t('Start date'))], [$register_date, new TLabel(_t('Time')), $time], [new TLabel(_t('Due date'))], [$close_date]);
